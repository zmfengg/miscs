'''
#! coding=utf-8
Original Source from: https://github.com/rnelsonchem/simpledbf
Because sometimes I need to read the last_n records out from
a dbf, so small tweaks were appended
...
class Dbf5(DbfBase):
    ...
    '''
    def __init__(self, dbf, codec='utf-8', last_n=None, cn_fmtr=lambda x: x):
        self._enc = codec
        ...
        for fieldno in range(self.numfields):
            ...
            fields.append((cn_fmtr(name.decode(self._enc)), typ.decode(self._enc), size))
        self.fields = fields
        ...
        # hack by zmFeng for getting the last_n records
        if last_n:
            last_n = abs(last_n)
            steps = (self.numrec - last_n) * self.fmtsiz
            if steps > 0:
                self.f.seek(steps, os.SEEK_CUR)
                self.numrec = last_n
